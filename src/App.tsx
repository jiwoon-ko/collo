...
function App() {
  ...
  const [arenaTab, setArenaTab] = useState<"brief" | "lab" | "guide">("brief");
  ...
  const applyRoute = () => {
    const [, route = "home", id, requestedTab] =
    ...
    if (nextView === "arena") {
      ...
      setArenaTab(
        requestedTab === "lab" || requestedTab === "guide"
          ? requestedTab
          : "brief",
      );
    }
  ...
  const openCoach = () => navigate("arena", challenges[0], "lab");
  const nav = [
  ...
  return (
    <div className="page home-page">
      ...
      <section className="coach-banner">
        ...
        <button className="outline-btn" onClick={openCoach}>
...
function Arena({
  ...
  onBack: () => void;
  initialTab: "brief" | "lab" | "guide";
}) {
  const [tab, setTab] = useState<"brief" | "lab" | "guide">(initialTab);
...
function AdminPanel({ notify }: { notify: (s: string) => void }) {
  const [topic, setTopic] = useState("Reflected search input");
  const [category, setCategory] = useState("XSS");
  const [difficulty, setDifficulty] = useState("Beginner");
  const [isGenerating, setIsGenerating] = useState(false);
  const [selectedSolution, setSelectedSolution] = useState<number | null>(null);
  const [arenas, setArenas] = useState<ArenaSolution[]>(() =>
    challenges.map((challenge) => ({
      ...
      flag:
        challenge.id === 1
          ? "flag{echo_reflection_found}"
          : `flag{${challenge.category.toLowerCase()}_training_complete}`,
      solution:
        challenge.id === 1
          ? "Trace the URL parameter to the innerHTML sink. The secure remediation is rendering it with textContent or a framework-escaped text node."
          : "AI-generated answer material is restricted to the administrator console. Review the vulnerable boundary, validate the flag, then publish only learner-safe hints.",
      hint:
        challenge.id === 1
          ? "Map the path from query parameter to rendering API."
          : "Inspect the trust boundary before changing any input.",
    })),
  );
  const active = arenas.find((arena) => arena.id === selectedSolution);
  const generateArena = () => {
    setIsGenerating(true);
    window.setTimeout(() => {
      const id = Date.now();
      const title = topic.trim() || `${category} Signal Lab`;
      const pointMap: Record<string, number> = {
        Beginner: 140,
        Intermediate: 260,
        Advanced: 420,
      };
      const newArena: ArenaSolution = {
        id,
        title,
        category,
        difficulty,
        points: pointMap[difficulty],
        flag: `flag{${category.toLowerCase().replace(/[^a-z]/g, "")}_signal_verified}`,
        solution: `AI solution for ${title}: identify the untrusted source, follow it to the ${category} boundary, and apply context-aware validation or safe APIs. The verification flag is generated with this draft and is visible only to administrators.`,
        hint: `Start by mapping the user-controlled value before it reaches the ${category} boundary.`,
        generated: true,
      };
      setArenas((items) => [newArena, ...items]);
      setSelectedSolution(id);
      setIsGenerating(false);
      notify("AI가 아레나 초안과 관리자 전용 해답지를 함께 생성했습니다.");
    }, 700);
  };
  return (
    <div className="page">
      <section className="page-title control-title">
        <div>
          ...
          <h1>Control room</h1>
          <p>
            AI가 학습용 아레나, 힌트, 검증 플래그와 관리자 전용 해답지를 한 번에
            생성합니다.
          </p>
        </div>
        <span className="admin-lock">
          <Icon name="lock" size={15} /> Admin-only workspace
        </span>
      </section>
      <div className="admin-grid">
        <section className="builder ai-builder">
          <div className="panel-title">
            <div>
              <span className="tiny-label">AI ARENA GENERATOR</span>
              <h2>새 아레나 생성</h2>
            </div>
            <span className="step">AI draft</span>
          </div>
          <p className="builder-copy">
            관리자는 주제와 난이도만 지정합니다. 풀이·플래그·검증 기준은 AI가
            초안으로 함께 생성하며, 학습자 화면에는 노출되지 않습니다.
          </p>
          <div className="form-grid">
            <label>
              ...
              <input
                ...
                placeholder="예: Reflected search input"
              />
            </label>
            <label>
              카테고리
            ...
            <label>
              생성 범위
              <input value="문제 · 힌트 · 플래그 · 해답지" readOnly />
            </label>
          </div>
          <button
            className="primary"
            onClick={generateArena}
            disabled={isGenerating}
          >
            {isGenerating ? "AI is creating…" : "AI로 아레나와 해답지 생성"}{" "}
            <Icon name="spark" size={16} />
          </button>
        </section>
        <aside className="api-card">
          <span className="tiny-label">ADMIN SAFEGUARD</span>
          <h3>Solutions stay private.</h3>
          <p>
            생성된 해답지는 이 Control room에서만 열립니다. 일반 사용자는 미션,
            제한된 힌트, 플래그 제출 화면만 볼 수 있습니다.
          </p>
          <div className="admin-scope">
            <Icon name="shield" size={16} />
            <span>Admin session · solution registry unlocked</span>
          </div>
          <small>
            <Icon name="lock" size={12} /> 실제 배포에서는 역할 기반 인증과 서버
            측 AI 호출을 연결하세요.
          </small>
        </aside>
      </div>
      <section className="solution-table">
        <div className="panel-title">
          ...
          <span className="registry-count">{arenas.length} arenas</span>
        </div>
        <div className="arena-registry">
          {arenas.map((arena) => (
            <button
              className={
                "arena-row " + (selectedSolution === arena.id ? "selected" : "")
              }
              key={arena.id}
              onClick={() => setSelectedSolution(arena.id)}
            >
              <span className="check-circle">✓</span>
              <span>
                <b>{arena.title}</b>
                <small>
                  {arena.category} · {arena.difficulty} · +{arena.points} SP
                  {arena.generated ? " · AI draft" : ""}
                </small>
              </span>
              <span className="verified">Verified</span>
              <span className="view-solution">View →</span>
            </button>
          ))}
        </div>
        {active && (
          <div className="admin-solution">
            <div>
              <span className="tiny-label">ADMIN-ONLY SOLUTION</span>
              <h3>{active.title}</h3>
              <p>{active.solution}</p>
              <div className="solution-meta">
                <span>
                  Verification flag <code>{active.flag}</code>
                </span>
                <span>Learner hint: {active.hint}</span>
              </div>
            </div>
            <button
              className="text-btn"
